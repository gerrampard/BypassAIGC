...
show_menu() {
    clear
    echo -e "\n${CYAN}========================================${NC}"
    echo -e "${CYAN}   AI 学术写作助手 - 统一管理脚本${NC}"
    echo -e "${CYAN}========================================${NC}\n"
    ...
    if command -v python3 &> /dev/null; then
        ...
        if [[ $python_version =~ Python[[:space:]]([0-9]+)\.([0-9]+) ]]; then
            ...
            if [ "$major" -ge 3 ] && [ "$minor" -ge 10 ]; then
                ...
                echo -e "${RED}× Python 版本需要 3.10 或更高，当前: $python_version${NC}"
                echo -e "${YELLOW}请更新 Python: sudo apt install python3.10 python3.10-venv${NC}"
                read -p "按回车键返回菜单..."
                return
            fi
        fi
        ...
        echo -e "${YELLOW}请安装 Python: sudo apt install python3 python3-venv python3-pip${NC}"
        read -p "按回车键返回菜单..."
        return
    fi
    
    # 检查 Node.js
    echo -e "\n${YELLOW}[2/5] 检查 Node.js...${NC}"
    if command -v node &> /dev/null; then
        ...
        if [[ $node_version =~ v([0-9]+) ]]; then
            ...
            if [ "$major" -ge 16 ]; then
                ...
                echo -e "${RED}× Node.js 版本需要 16 或更高，当前: $node_version${NC}"
                echo -e "${YELLOW}请更新 Node.js: https://nodejs.org/${NC}"
                read -p "按回车键返回菜单..."
                return
            fi
        fi
...
start_all_services() {
    ...
    if command -v tmux &> /dev/null; then
        ...
        echo -e "${CYAN}安装命令: sudo apt install tmux${NC}"
...
verify_installation() {
    echo -e "\n${CYAN}========================================${NC}"
    echo -e "${CYAN}验证安装状态...${NC}"
    echo -e "${CYAN}========================================${NC}\n"
    
    local all_ok=true
    
    # 检查 Python
    echo -e "${YELLOW}[1/7] 检查 Python...${NC}"
    if command -v python3 &> /dev/null; then
        python_version=$(python3 --version 2>&1)
        echo -e "${GREEN}✓ $python_version${NC}"
    else
    ...
    # 检查 Node.js
    echo -e "${YELLOW}[2/7] 检查 Node.js...${NC}"
    if command -v node &> /dev/null; then
        node_version=$(node --version 2>&1)
        echo -e "${GREEN}✓ Node.js $node_version${NC}"
    else
